...
class Connect4:
    ...
    def __init__(self):
        """
        The constructor creates an instance of the Connect4 class and initializes an empty board.
        """
        self.clear_board()
    ...
    def __current_player_checker(self):
        """
        Get the checker associated with the current player.
        """
    ...
    def clear_board(self):
        ...
        **NOT SURE IF THIS SHOULD BE INCLUDED OR NOT, BECAUSE THE USER COULD JUST CREATE A NEW CONNECT4 GAME EACH TIME**
        ...
        Clear the board of all pieces, effectively restarting the game.
        """
        for _ in range(self._BOARD_HEIGHT):
            self._board.append([Checker.EMPTY] * self._BOARD_WIDTH)
    ...
    def __update_game_state(self):
        ...
        for i in range(self._BOARD_HEIGHT):
            ...
            for j in range(self._BOARD_WIDTH - self._WIN_LENGTH + 1):
                if all([checker is self.__current_player_checker() for checker in self._board[i][j:j+self._WIN_LENGTH]]):
        ...
        for i in range(self._BOARD_HEIGHT - self._WIN_LENGTH + 1):
            for j in range(self._BOARD_WIDTH):
                if all([row[j] is self.__current_player_checker() for row in self._board[i:i+self._WIN_LENGTH]]):
                    self._game_state = self.__current_player_win_game_state()
                    return
        ...
        for i in range(0, self._BOARD_HEIGHT - self._WIN_LENGTH + 1):
            for j in range(0, self._BOARD_WIDTH - self._WIN_LENGTH + 1):
                ...
                if all([self._board[i+index][j+(self._WIN_LENGTH - 1 - index)] is self.__current_player_checker() for index in range(self._WIN_LENGTH)]) or \
                   all([self._board[i+index][j+index] is self.__current_player_checker() for index in range(self._WIN_LENGTH)]):
